...
import net.kxmischesdomi.customitems.utils.customitems.CustomItemUtils;
...
public class CustomItemRecipeMenu implements MenuPosition {
	...
	private List<Recipe> getRecipes(@Nonnull ItemStack itemStack) {
		...
		if (item.getRecipe() == null) return new ArrayList<>();
		return item.getRecipe().getRecipeBuilders().stream().map(RecipeBuilder::getRecipe).collect(Collectors.toList());
	}
